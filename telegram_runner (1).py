#!/usr/bin/env python3
"""
Telegram Bot Runner - Connected to LeadGen Pro
...
def build_signature(campaign_sig, global_sig):
    """Build signature from campaign or global settings"""
    parts = []
...
async def init_client(config):
    ...
    if not api_id or not api_hash:
        print(f"{RED}ERROR: Telegram API credentials not set.{RESET}")
        return None

    try:
        client = TelegramClient("telegram_runner_session", int(api_id), api_hash)
