...
<style>
    ...
    .chat-message-user {
        ...
        margin: 6px 0 6px auto; max-width: 80%; font-size: 14px; line-height: 1.5;
    }
    .chat-message-ai {
        background: linear-gradient(145deg, rgba(255,255,255,0.1) 0%, rgba(255,255,255,0.05) 100%);
        border: 1px solid rgba(255,255,255,0.08); color: #e5e7eb;
        border-radius: 18px 18px 18px 4px; padding: 12px 16px;
        margin: 6px auto 6px 0; max-width: 85%; font-size: 14px; line-height: 1.6;
    }
    .test-result-success {
        background: rgba(34,197,94,0.1); border: 1px solid rgba(34,197,94,0.3);
        border-radius: 10px; padding: 14px; color: #86efac;
    }
    .test-result-fail {
        background: rgba(239,68,68,0.1); border: 1px solid rgba(239,68,68,0.3);
        border-radius: 10px; padding: 14px; color: #fca5a5;
    }
    .stTabs [data-baseweb="tab-list"] {
        gap: 8px; background: rgba(255,255,255,0.03); border-radius: 12px; padding: 6px;
    }
    .stTabs [data-baseweb="tab"] {
        border-radius: 8px; padding: 10px 20px; font-weight: 500; color: #9ca3af;
    }
...
PLATFORMS = {
    "telegram": {
        ...
        "fields": {
            "api_id": {"label": "API ID", "type": "text", "required": True, "help": "From my.telegram.org"},
            "api_hash": {"label": "API Hash", "type": "password", "required": True, "help": "Long alphanumeric string"},
            "phone": {"label": "Phone Number", "type": "text", "required": True, "help": "Format: +1234567890"},
            "session_name": {"label": "Session Name", "type": "text", "required": False, "help": "Optional"}
        },
    ...
    "reddit": {
        ...
        "fields": {
            "client_id": {"label": "Client ID", "type": "text", "required": True, "help": "Under app name on prefs/apps"},
            "client_secret": {"label": "Client Secret", "type": "password", "required": True, "help": "Click 'edit' to reveal"},
    ...
    "twitter": {
        ...
        "fields": {
            ...
            "access_secret": {"label": "Access Secret", "type": "password", "required": True}
        },
    ...
    "instagram": {
        ...
        "fields": {
            "username": {"label": "Username", "type": "text", "required": True},
            "password": {"label": "Password", "type": "password", "required": True},
            "session_id": {"label": "Session ID", "type": "text", "required": False}
        },
...
def process_ai_message(user_input: str, api_key: str):
    """Process a single AI message — add to history, call API, add response."""
    ...
    try:
        ...
        conn.close()
        context["platforms"] = {pid: {"configured": is_platform_configured(pid)} for pid in PLATFORMS}
    except:
        pass
    ...
    for i in range(0, len(st.session_state.chat_history) - 1, 2):
        if i + 1 < len(st.session_state.chat_history):
            history.append({
                ...
                "ai": st.session_state.chat_history[i + 1]["content"]
            })
...
def main():
    ...
    # ── AI ASSISTANT ────────────────────────────────────────────────────────
        ...
        if st.session_state.last_error:
            ...
            if st.button("🗑️ Dismiss Error", key="dismiss_err"):
                st.session_state.last_error = None
                st.rerun()

        st.markdown("<div style='height:12px;'></div>", unsafe_allow_html=True)

        # Process pending from suggestion buttons
        if "pending_msg" in st.session_state and st.session_state.pending_msg:
            msg_to_send = st.session_state.pending_msg
            st.session_state.pending_msg = ""
            if api_key:
                process_ai_message(msg_to_send, api_key)
            st.rerun()
        ...
        for i, sug in enumerate(suggestions):
            with sug_cols[i % 2]:
                if st.button(sug, key=f"sug_{i}", use_container_width=True):
                    st.session_state.pending_msg = sug
                    st.rerun()

        # Form prevents auto-resubmit
        with st.form(key="chat_form", clear_on_submit=True):
            user_input = st.text_input("Message", key=f"chat_input_{st.session_state.input_key}",
                placeholder="Ask anything...", label_visibility="collapsed")
            submitted = st.form_submit_button("Send", use_container_width=True)

        if submitted and user_input.strip() and api_key:
            process_ai_message(user_input.strip(), api_key)
            st.session_state.input_key += 1
            st.rerun()
        elif submitted and user_input.strip() and not api_key:
            st.error("Please configure your API key in Settings first.")
                ...
                try:
                    ...
                    if st.button("📥 Import", use_container_width=True):
                        ...
                        for _, r in df.iterrows():
                            plat = str(r.get("platform","")).strip().lower()
