...
def main():
    ...
    with st.sidebar:
        ...
        <div style="text-align: center; padding: 10px 0 20px 0;">
            ...
            <div style="font-size: 0.75rem; color: #6b7280;">v4.0</div>
        </div>
        """, unsafe_allow_html=True)
    ...
    # ── AI ASSISTANT ────────────────────────────────────────────────────────
        ...
        if not ai_engine.is_configured():
            ...
            with col2:
                ...
                if api_key_input and st.button("💾 Save Key", use_container_width=True):
                    ...
                    save_config(cfg)
                    st.success("API key saved! Refresh the page to activate AI.")
                    st.rerun()
        ...
        if "ai_engine" not in st.session_state:
            ...
            cfg = load_config()
            saved_key = cfg.get("settings", {}).get("groq_api_key", "")
            st.session_state.ai_engine = AIChatEngine(api_key=saved_key)
